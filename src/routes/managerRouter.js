...
const {
  ...
  findManagerByName,
  updateManager,
} = require("../dbQueries/managerQueries");
...
managerRouter.post("/sign-up", [], async (req, res) => {
  ...
  if ((await manager.errorCode) === "P2002") {
    return await res
      .status(400)
      .json({ flag: false, data: {}, message: "Existed managerName" });
  }
  ...
  if (await manager.errorCode) {
    return await res.status(400).json({
      flag: false,
      data: {},
      message: `Failed! Error : ${manager.errorCode}`,
    });
  }

  return await res
    .status(201)
    .json({ flag: true, data: {}, message: "New manager added" });
});
...
managerRouter.post("/login", [], async (req, res) => {
  ...
  if (!manager) {
    return await res
      .status(404)
      .json({ flag: false, data: {}, message: "Manager was not found" });
  }
  ...
  return await res.status(200).json({
    flag: true,
    data: { tokenData: generateToken({ id: manager.id }) },
...
managerRouter.post("/update", [authMiddleware], async (req, res) => {
  ...
  if ((await manager.id) !== res.locals.authId) {
    return await res
      .status(403)
      .json({ flag: false, data: {}, message: "Incorrect manager id" });
  }
  ...
  return await res
    .status(200)
    .json({ flag: true, data: {}, message: "Manager is updated !" });
});
